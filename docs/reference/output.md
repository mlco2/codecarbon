...
The **Provenance** column says what produced each number: whether it was read from a
hardware counter, modelled from a constant, or copied from configuration. This matters —
a modelled column and a measured column can differ by a large factor on the same
machine, and nothing else in the CSV tells you which one you got.

| Field | Description | Provenance |
|-------|-------------|------------|
| timestamp | Time of the experiment in `%Y-%m-%dT%H:%M:%S` format | Local clock at write time |
...
| emissions | Emissions as CO₂-equivalents (CO₂eq), in kg | Computed: `energy × carbon intensity` — see note 3 |
| emissions_rate | Emissions divided per duration, in Kg/s | Computed: `emissions / duration` |
| cpu_power | Mean CPU power (W) | **Varies by backend** — see note 1. Mean of the per-interval samples, *not* PUE-scaled |
...
| ram_power | Mean RAM power (W) | **Always modelled, never measured** — see note 2 |
| cpu_energy | Energy used per CPU (kWh) | Same backend as `cpu_power`, **PUE-inflated** — see notes 1 and 4 |
...
| ram_energy | Energy used per RAM (kWh) | Modelled RAM power × interval, **PUE-inflated** — see note 2 |
...
| python_version | Python version (e.g., 3.8.10) | `platform` module |
| codecarbon_version | Version of codecarbon used | Package metadata |
...
| longitude | Longitude of the machine | IP geolocation, full precision — see note 5 |
| latitude | Latitude of the machine | IP geolocation, full precision — see note 5 |
...
**1. `cpu_power` and `cpu_energy` depend entirely on the backend selected at startup.**
The backend is chosen once, logged at startup, and not recorded in the CSV. Roughly in
...
**2. `ram_power` and `ram_energy` are never measured.** Commodity hardware exposes no
RAM energy counter. CodeCarbon estimates a DIMM count from the total RAM size, then
applies 5 W per DIMM on x86 (1.5 W on ARM), with decreasing marginal power above four
DIMMs and a floor of two DIMMs' worth (`external/ram.py:82-193`). Treat these columns as
an order-of-magnitude heuristic. If you can measure your own RAM power, override it with
`force_ram_power`.

**3. `emissions` is only as good as the carbon intensity behind it.** The intensity comes
from a fallback chain (`core/emissions.py`): a forced value, then the Electricity Maps
API if a token is set, then regional data (USA/CAN/SWE/NOR/FIN), then country energy-mix
data, and finally the world average of 475 gCO₂eq/kWh when nothing matched
(`core/emissions.py:88-96`, `:303-315`). On cloud, a region miss falls back to the
country value. **No column records which level answered** — check the run's log output if
you need to know.

**4. PUE multiplies the per-component energy columns, not just the total.** At
`emissions_tracker.py:1194` the PUE is applied to each hardware measurement *before* it
is accumulated. With `pue=1.5`, the `cpu_energy` column is therefore 1.5× the energy the
CPU actually drew: it is datacenter energy attributed to the CPU, not raw CPU energy. The
power columns are **not** scaled by PUE, so `cpu_energy` will not equal
`cpu_power × duration` when `pue != 1`. `water_consumed` is computed from the
already-inflated energy.
