...
code. You just need to copy-paste [from codecarbon import
track_emissions]{.title-ref} and add the [\@track_emissions]{.title-ref}
decorator to your training function. The emissions will be tracked
...
: you instantiate the tracker and call the [start()]{.title-ref} method
...
[try\...finally]{.title-ref} block to ensure that the tracker is stopped
even if an error occurs during training. This is important to ensure the
CodeCarbon scheduler is stopped. If you don\'t use
[try\...finally]{.title-ref}, the scheduler will continue running in the
