Carbon Offsetting with 1ClickImpact
====================================

CodeCarbon integrates with `1ClickImpact <https://1clickimpact.com>`_ to provide automatic carbon offsetting capabilities. This allows you to not only track your carbon emissions but also automatically offset them through verified capture carbon projects.
...
- **Carbon Capture**: Offsets are performed through 1ClickImpact's carbon capture projects
...
2. Get a 1ClickImpact API key from `1ClickImpact.com <https://1clickimpact.com>`_

3. Configure your tracker with 1ClickImpact parameters
...
.. code-block:: python
   ...
   tracker = EmissionsTracker(
       ...
       offset_api_key="your_api_key_here", # starts with "live_" for production, "test_" for sandbox
...
.. list-table:: 1ClickImpact Parameters
   :widths: 25 75
   :header-rows: 1
   ...
   * - Parameter
     - Description
   * - offset_api_key
     - Your 1ClickImpact API key (required for offsetting)
   * - offset_environment
...
You can also configure 1ClickImpact parameters in your `.codecarbon.config` file:
...
3. **Conversion**: Emissions are converted from kg CO2 to pounds (lbs) for the 1ClickImpact API
4. **Capture Carbon**: 1ClickImpact's `capture_carbon()` method is called to offset the emissions
...
- Offsetting costs vary based on the amount of CO2 and offset_threshold
- Consider setting appropriate thresholds for your use case
- Monitor your offset costs through 1ClickImpact's platform (https://1clickimpact.com)
...
**Failed to initialize MakeImpact SDK**
...
For more information, visit the `1ClickImpact documentation <https://docs.1clickimpact.com>`_.
