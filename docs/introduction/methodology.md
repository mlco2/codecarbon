...
its electricity mix, we apply a world average of 475 gCO2.eq/KWh (
...
it using the `force_ram_power` parameter, which will
...
20 Watts, just add `force_ram_power=20` to the init of
CodeCarbon.
...
![CPU Fallback](../images/cpu_fallback.png){.align-center}
