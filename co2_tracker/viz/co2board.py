...
def render_app(df: pd.DataFrame):
    ...
    graph = dcc.Graph(
        figure=px.line(
            df, x="timestamp", y="emissions", template="seaborn"
        ),
    )
    ...
    default_comparisons = html.Div(
        [
            ...
            html.Div(
                id="emission-comparisons-header",
                style={
                    "textAlign": "left",
                }
            ),
                    ...
                    dbc.Col(
                        [
                            ...
                            dash_table.DataTable(
                                id='energy-mix-table',
                                columns=[
                                    {"name": "Location", "id": "location", 'deletable': False, 'renamable': False},
                                    {"name": "Coal", "id": "coal", 'deletable': False, 'renamable': False},
                                    {"name": "Oil/Petroleum", "id": "oil-petroleum", 'deletable': False, 'renamable': False},
                                    {"name": "Natural Gas", "id": "natural-gas", 'deletable': False, 'renamable': False},
                                    {"name": "Low Carbon", "id": "low-carbon", 'deletable': False, 'renamable': False},
                                ],
                                editable = False,
                            )
                        ],
    ...
    app.layout = dbc.Container([header, project_dropdown, cards, default_comparisons,
                                custom_comparison, graph_and_table, emoticon_credit,
                                hidden_div1, hidden_div2],
    ...
    def update_project(project_name):
        ...
        if country not in countries_with_regional_energy_mix and country in country_list:
    ...
    def update_equivalencies_card(jsonified_variables):
        ...
        source1 = [
            html.Img(src=app.get_asset_url('mountain_pickax.png'), alt="coal image", width="32", height="32"),
            "{}: ".format(energy_sources[0]),
            html.Strong(["996 kg CO",html.Sub("2"),"/MWh"], style={"color": "green"})
        ]
        source2 = [
            html.Img(src=app.get_asset_url('oil.png'), alt="oil/petroleum image", width="32", height="32"),
            "{}: ".format(energy_sources[1]),
            html.Strong(["817 kg CO",html.Sub("2"),"/MWh"], style={"color": "green"})
        ]
        source3 = [
            html.Img(src=app.get_asset_url('natural_gas.png'), alt="natural gas image", width="32", height="32"),
            "{}: ".format(energy_sources[2]),
            html.Strong(["744 kg CO",html.Sub("2"),"/MWh"], style={"color": "green"})
        ]
        source4 = [
            html.Img(src=app.get_asset_url('low_carbon.png'), alt="low carbon image", width="32", height="32"),
        ...
        if country in countries_with_regional_energy_mix:
            ...
            location = country
        energy_mix = report.energy_mix_graph(location, countries_with_regional_energy_mix)
        ...
        return (source1, source2, source3, source4, energy_mix, car_img_src,
                car_img_alt, car_img_width, car_img_height, tv_img_src,
                tv_img_alt, tv_img_width, tv_img_height, house_img_src,
                house_img_alt, house_img_width, house_img_height)
    ...
    def update_custom_comparison_graph(countries, regions, jsonified_variables):
        ...
        for location in locations:
            labels, values = report.energy_mix_data(location, countries_with_regional_energy_mix)
            if location == locations[-1]:
                most_recent_mix_graph = report.energy_mix_graph(location, countries_with_regional_energy_mix)
                chosen_region_pie_title = "Energy Mix: {}".format(location)
            mix_data.append({"location": location, "coal": "{}%".format(values[0]),
                            "oil-petroleum": "{}%".format(values[1]), "natural-gas": "{}%".format(values[2]),
