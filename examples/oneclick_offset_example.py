#!/usr/bin/env python3
"""
Example demonstrating CodeCarbon integration with 1ClickImpact for automatic carbon offsetting.

This example shows how to:
1. Track carbon emissions from code execution
2. Automatically offset emissions through 1ClickImpact's carbon capture projects when thresholds are reached
3. Configure offset thresholds (minimum 1 kg CO2) and environments (sandbox or production)

Requirements:
- pip install codecarbon[offset]
- 1ClickImpact API key (get from https://1clickimpact.com)
...
def example_with_tracker():
    """
    Example using EmissionsTracker object with 1ClickImpact integration
    """
    print("\n=== Example 1: Using EmissionsTracker with 1ClickImpact ===")

    # Initialize tracker with 1ClickImpact settings
    tracker = EmissionsTracker(
        project_name="oneclick_demo",
        # 1ClickImpact configuration
...
def example_with_decorator():
    """
    Example using @track_emissions decorator with 1ClickImpact integration
    """
    print("\n=== Example 2: Using @track_emissions decorator with 1ClickImpact ===")
...
def example_manual_offset():
    ...
    try:
        ...
        # Access the 1ClickImpact output handler for manual control
        ...
        for handler in tracker._output_handlers:
            if isinstance(handler, OneClickImpactOutput):
                print(
                    f"Accumulated emissions: {handler.accumulated_emissions_kg:.6f} kg CO2"
                )
            ...
            print("1ClickImpact handler not found - check API key configuration")
...
if __name__ == "__main__":
    print("1ClickImpact Carbon Offset Integration Examples")
    print("=" * 50)
    print("\nNOTE: These examples use 'sandbox' environment.")
    print("For real carbon offsetting, change 'offset_environment' to 'production'")
    print("and replace 'your_api_key_here' with your actual 1ClickImpact API key.")
    ...
    try:
        ...
        if "makeimpact" in str(e):
            print("ERROR: MakeImpact SDK not installed.")
