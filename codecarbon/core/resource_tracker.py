...
class ResourceTracker:
    ...
    def set_RAM_tracking(self, tracker):
        """
        Set up RAM tracking based on the user's configuration.
        param tracker: BaseEmissionsTracker object
        """
        logger.info("[setup] RAM Tracking...")

        ram = RAM(tracking_mode=tracker._tracking_mode)
        tracker._conf["ram_total_size"] = ram.machine_memory_GB
        tracker._hardware: List[Union[RAM, CPU, GPU, AppleSiliconChip]] = [ram]
        self.ram_tracker = "3 Watts for 8 GB ratio constant"

    def set_CPU_tracking(self, tracker):
        logger.info("[setup] CPU Tracking...")
        if cpu.is_powergadget_available() and tracker._default_cpu_power is None:
            logger.info("Tracking Intel CPU via Power Gadget")
            self.cpu_tracker = "Power Gadget"
            hardware = CPU.from_utils(tracker._output_dir, "intel_power_gadget")
            tracker._hardware.append(hardware)
            tracker._conf["cpu_model"] = hardware.get_model()
        elif cpu.is_rapl_available():
            logger.info("Tracking Intel CPU via RAPL interface")
            self.cpu_tracker = "RAPL"
            hardware = CPU.from_utils(tracker._output_dir, "intel_rapl")
            tracker._hardware.append(hardware)
            tracker._conf["cpu_model"] = hardware.get_model()
        elif (
            powermetrics.is_powermetrics_available()
            and tracker._default_cpu_power is None
        ):
            ...
            hardware_cpu = AppleSiliconChip.from_utils(
                tracker._output_dir, chip_part="CPU"
            )
            tracker._hardware.append(hardware_cpu)
            tracker._conf["cpu_model"] = hardware_cpu.get_model()

            hardware_gpu = AppleSiliconChip.from_utils(
                tracker._output_dir, chip_part="GPU"
            )
            tracker._hardware.append(hardware_gpu)

            tracker._conf["gpu_model"] = hardware_gpu.get_model()
            tracker._conf["gpu_count"] = 1
        else:
            ...
            power = tdp.tdp
            model = tdp.model
            if (power is None) and tracker._default_cpu_power:
                # We haven't been able to calculate CPU power but user has input a default one. We use it
                user_input_power = tracker._default_cpu_power
            ...
            if tdp:
                if cpu.is_psutil_available():
                    ...
                    hardware = CPU.from_utils(
                        tracker._output_dir, "constant", model, power
                    )
                tracker._hardware.append(hardware)
            else:
                if cpu.is_psutil_available():
                    ...
                    hardware = CPU.from_utils(tracker._output_dir, "constant")
        if tracker._conf.get("force_mode_cpu_load", False):
            # ########### DEBUG ################
    ...
    def set_GPU_tracking(self, tracker):
        """
        Set up GPU tracking based on the user's configuration.
        param tracker: BaseEmissionsTracker object
        """
        logger.info("[setup] GPU Tracking...")
        if tracker._gpu_ids:
            # If _gpu_ids is a string or a list of int, parse it to a list of ints
            if isinstance(tracker._gpu_ids, str) or (
                isinstance(tracker._gpu_ids, list)
                and all(isinstance(gpu_id, int) for gpu_id in tracker._gpu_ids)
            ):
                tracker._gpu_ids: List[int] = parse_gpu_ids(tracker._gpu_ids)
                tracker._conf["gpu_ids"] = tracker._gpu_ids
                tracker._conf["gpu_count"] = len(tracker._gpu_ids)
            else:
                logger.warning(
        ...
        if gpu.is_gpu_details_available():
            ...
            gpu_devices = GPU.from_utils(tracker._gpu_ids)
            tracker._hardware.append(gpu_devices)
            gpu_names = [n["name"] for n in gpu_devices.devices.get_gpu_static_info()]
            gpu_names_dict = Counter(gpu_names)
            tracker._conf["gpu_model"] = "".join(
                [f"{i} x {name}" for name, i in gpu_names_dict.items()]
            )
            if tracker._conf.get("gpu_count") is None:
                tracker._conf["gpu_count"] = len(
