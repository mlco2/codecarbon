...
class WaterConsumption:
    ...
    def get_cloud_water_consumption(
        ...
        df: pd.DataFrame = self._data_source.get_cloud_water_consumption_data()
        try:
            water_consumption_per_kWh: WaterPerKWh = WaterPerKWh.from_g_per_kWh(
                df.loc[
                    (df["provider"] == cloud.provider) & (df["region"] == cloud.region)
                ]["impact"].item()
            )
            water_consumption = water_consumption_per_kWh.l_per_kWh * energy.kWh  # L
        except Exception as e:
            logger.warning(
                f"Cloud water usage effectiveness for provider '{cloud.provider}' and region '{cloud.region}' not found, using country value instead. Error : {e}"
            )
            logger.warning(
