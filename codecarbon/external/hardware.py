            ...
            for gpu_id in self.gpu_ids:
                ...
                if isinstance(gpu_id, int):
                    if 0 <= gpu_id < self.num_gpus:
                        ...
                        found_gpu_id = True
                # Check if it looks like an index and if it's valid for the number of GPUs on the system
                elif gpu_id.isdigit():
                    if 0 <= int(gpu_id) < self.num_gpus:
                        monitored_gpu_ids.append(int(gpu_id))
                        found_gpu_id = True
