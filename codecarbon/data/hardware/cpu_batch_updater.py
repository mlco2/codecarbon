...
def update_amd_files():
    ...
    try:
        ...
        pd.DataFrame(desktop_laptop).to_csv(
            os.path.join(DATA_DIR, 'AMD_CPU_desktop_laptop.csv'), 
            index=False
        )
...
def validate_updates():
    ...
    try:
        ...
        assert os.path.exists(os.path.join(DATA_DIR, 'AMD_Server_Processor_Specifications.csv'))
...
def update_all_cpus():
    ...
    combined.to_csv(
        os.path.join(DATA_DIR, 'cpu_power.csv'), 
        index=False
    )
