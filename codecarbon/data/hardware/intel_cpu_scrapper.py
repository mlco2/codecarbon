...
class IntelCpuScraper:
    ...
    def parse_html(self, html_content):
        ...
        if not rows:
            ...
            if no_results_message and "no results found" in no_results_message.text.lower():
        ...
        for row in rows:
            try:
                ...
                launch_date = get_cell_text(1)
                
                # 3. Total Cores (e.g., "40")
                total_cores = get_cell_text(2)
                
                # 4. Max Turbo Frequency (e.g., "3.5 GHz")
                max_turbo_frequency = get_cell_text(3)
                
                # 5. Processor Base Frequency (e.g., "2.5 GHz")
                processor_base_frequency = get_cell_text(4)
                
                # 6. Cache (e.g., "160 MB")
                cache = get_cell_text(5)
                
                # 7. TDP (e.g., "235 W")
                tdp_text_content = get_cell_text(6)
                tdp_data_value = tds[6].get('data-value') if len(tds) > 6 and tds[6] else None
                ...
                result_item = {
                    ...
                    'cache': cache,
                    'tdp': tdp_numeric if tdp_numeric is not None else "" # Store numeric TDP in Watts
                }
    ...
    def scrape_all_cpus(self):
        ...
        max_consecutive_empty = 2 # Stop after 2 consecutive pages with no new valid entries
        ...
        while True:
            ...
            if page_results is None: # Indicates an unexpected page structure or error
                print(f"Could not parse data from page {page_num} or unexpected structure. Stopping.")
                break
            
            if not page_results:
                consecutive_empty_pages += 1
                print(f"No results extracted from page {page_num}. Consecutive empty pages: {consecutive_empty_pages}")
                ...
                for res in page_results:
                    if res not in self.results: # Avoid duplicates if any page re-processing happens
                        self.results.append(res)
                        new_cpus_found_on_page +=1
                print(f"Added {new_cpus_found_on_page} new CPUs from page {page_num}. Total CPUs: {len(self.results)}")
                if new_cpus_found_on_page == 0 and len(page_results) > 0:
                    ...
                    print(f"All {len(page_results)} CPUs on page {page_num} are duplicates. Considering this as end of new data.")
                    break
