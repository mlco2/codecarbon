...
class OneClickImpactOutput(BaseOutput):
    ...
    def manual_offset(self, emissions_kg: Optional[float] = None) -> bool:
        """
        Manually trigger offset for accumulated or specified emissions
        
        :param emissions_kg: Specific amount to offset. If None, offsets accumulated emissions
        :return: True if offset was successful, False otherwise
        """
        if emissions_kg is None:
            emissions_kg = self.accumulated_emissions_kg
            
        if emissions_kg <= 0:
            logger.warning("No emissions to offset")
            return False
            
        if self._offset_emissions(emissions_kg):
            if emissions_kg == self.accumulated_emissions_kg:
                self.accumulated_emissions_kg = 0.0
            return True
        return False
