    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip bg-background text-foreground p-2 border border-border rounded shadow">
          <p>
            <strong>Emissions:</strong> {data.emissions.toFixed(2)} kgeqCO2
          </p>
          <p>
            <strong>Energy Consumed:</strong> {data.energy_consumed.toFixed(2)}{" "}
            kWh
          </p>
          <p>
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>Click a run to see time series</CardDescription>
        </div>
        {!isPublicView && (
          <ExportCsvButton
