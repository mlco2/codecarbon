          ...
          experimentsReportData.map(async (exp) => {
            ...
            const runsWithDetails = await Promise.all(
              runs.map(async (run) => {
                ...
                const emissionsData = await getEmissionsTimeSeries(run.runId);

                // Return run with metadata and emissions
                return {
                  ...run,
                  emissions_value: run.emissions,
                  emissions: emissionsData?.emissions || undefined,
                  metadata: emissionsData.metadata || undefined,
                };
              }),
            );
  ...
  const headerContent = (
    <div className="flex flex-row gap-6 items-center justify-between w-full">
      <div className="flex items-center gap-2">
        <h1 className="text-2xl font-semi-bold">Project {project.name}</h1>
      ...
      <div className="flex items-center gap-2">
        ...
        <ShareProjectButton projectId={project.id} isPublic={project.public} />
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
