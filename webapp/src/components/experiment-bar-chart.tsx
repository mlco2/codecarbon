...
export default function ExperimentsBarChart({
  ...
  const CustomBar = useMemo(
    () => (props: any) => {
      ...
      const barFill =
        selectedBar === index ? "var(--color-desktop)" : "var(--color-mobile)";
      return (
        <rect
          x={x}
          y={y}
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Click an experiment to see the runs on the chart on the right
          </CardDescription>
        </div>
        {!isPublicView && (
          <ExportCsvButton
            isDisabled={isExporting || experimentsReportData.length === 0}
            onDownload={async () => {
              setIsExporting(true);
              exportExperimentsToCsv(experimentsReportData, projectName);
        ...
        <ChartContainer config={chartConfig}>
          {experimentsReportData.length > 0 ? (
            <BarChart accessibilityLayer data={experimentsReportData}>
              ...
              />
              <Bar dataKey="emissions" shape={<CustomBar />} radius={4} />
            </BarChart>
          ) : (
