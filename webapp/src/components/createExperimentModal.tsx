...
export default function CreateExperimentModal({
  ...
  const [createdExperiment, setCreatedExperiment] = useState<Experiment | null>(
    null,
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(`Experiment ${experimentData.name} created successfully`);
    } catch (error) {
            ...
            <div className="py-4">
              ...
              <div className="bg-muted flex items-center p-2 rounded justify-between">
                ...
                <Button
                  ...
                  onClick={() => handleCopy(createdExperiment?.id)}
                  className="ml-2"
                >
                  {isCopied ? (
