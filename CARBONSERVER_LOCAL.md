...
Management endpoints require a Bearer JWT. In local (`ENVIRONMENT=develop`) mode the signature is **not** verified — any well-formed JWT with `sub` and `email` claims is accepted.

Run the bootstrap script once the container is up — it generates a JWT and registers the user, printing the Bearer token to use in Swagger:

```bash
docker exec carbonserver python /app/carbonserver/bootstrap_local.py
```

> `PyJWT` and `requests` are already installed in the container image — no extra `pip install` needed.

---

### Manual bootstrap (alternative)

If you prefer to run the steps by hand, use the snippet below from any Python environment with `pyjwt` installed (`pip install pyjwt`):
