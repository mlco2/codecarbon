...
from charts import fig_co2_energy, fig_energy_by_day_period
...
def graph(identifier, figure=None):
    ...
    if figure:
        graph = dcc.Graph(
            id=identifier, figure=figure, config={"displayModeBar": False}
        )
    else:
        # A graph with callbacks
        graph = dcc.Graph(id=identifier, config={"displayModeBar": False})
    return graph
...
def section_header():
    ...
    component = html.Div(
        children=[
            html.H1(
                children="Evaluate CO2 Emissions",
                className="header-title",
            ),
            html.P(
                children="Analyze CO2 emissions generated"
...
def subsection_static_graphs(data):
    ...
    graphs = [
        ...
        graph(
            "graph_co2_energy",
            fig_co2_energy(
                data["energy_consumed"], data["emissions"], data["timestamp"]
            ),
        ),
        html.Br(),
    ]
